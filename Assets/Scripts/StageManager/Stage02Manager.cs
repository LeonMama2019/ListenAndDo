...
public class Stage02Manager : MonoBehaviour
{
    ...
    [SerializeField] private AnswerStage02 stage02Answer;

    [Header("Hit選択後、問題を開始するまでの待ち時間")]
    ...
    private void Start()
    {
        // Stage02はHit選択チュートリアル完了まで問題を開始しない。
    }

    public void StartQuestionsAfterHitSelected()
    {
    ...
    public void ShowNextQuestion()
    {
        ...
        {
            Debug.LogWarning("Stage02Manager: StageDataに問題がありません");
            return;
        }
        ...
        if (verbsController != null)
            verbsController.SetVerb(currentTask.verb, currentTask);

        if (stage02Answer != null)
            stage02Answer.SetTask(currentTask);
    ...
    {
        if (imageData == null || imageData.answerImages == null || imageData.answerImages.Length == 0)
            return null;

        Sprite randomSprite;
        do
        {
            randomSprite = imageData.answerImages[Random.Range(0, imageData.answerImages.Length)];
        }
    ...
    {
        if (task == null || object1 == null || object2 == null)
            return;
